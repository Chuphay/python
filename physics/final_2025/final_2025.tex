...
In \textbf{Section 2}, we introduce the core hypothesis: that mass acts as a sink for momentum from a pervasive background radiation field. This idea is presented intuitively, offering a physical picture that anchors the theory.

In \textbf{Sections 3 and 4}, we lay out the core physical assumptions underlying the hypothesis. By “assumptions,” we mean elements already present in the scientific literature—perhaps unconventional, but not invented for this theory. Section 3 adopts a Maxwell-style framework for gravity, building on gravitoelectromagnetism. Section 4 introduces the stochastic radiation field—drawn from stochastic electrodynamics—as the physical medium from which mass absorbs momentum.
...
This is the hypothesis that we have chosen and we will see it in action in the section on predicitons where we will use it like so:
...
And again this is the hypothesis of the paper that mass creates a momentum flucx that can act on radiation.

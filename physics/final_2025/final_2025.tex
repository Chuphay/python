...
More precisely, we hypothesize that mass acts as a \emph{sink} for zero-point radiation, a background field believed to pervade all of space.
...
We began with a question: Can gravity emerge from classical field dynamics, without invoking curvature or quantization? Modern physics remains split between two towering theories—quantum mechanics and general relativity—that refuse to merge. This paper offered a new hypothesis: that mass interacts with a pervasive zero-point radiation field, drawing in momentum and setting up a directional flow. From that single idea, a momentum field arises that shapes trajectories and mimics gravity.

Much of the work that followed was technical. We built a Maxwell-style classical framework, introduced physical cutoffs, computed energy balances, and checked for internal consistency. Static mass–mass attraction emerged from a Coulomb-like potential. At higher velocities, gravitomagnetic effects followed directly from the field equations—allowing us to recover phenomena such as the precession of Mercury. Most strikingly, we found that the same equations, when reframed, yield the Dirac equation. This suggests that quantum structure may be embedded within classical field theory, if the hypothesis is correct.

\subsection{What This Is—and Isn’t}

This is not a quantum theory. It does not resolve Bell’s theorem, explain entanglement, or reconstruct the Standard Model. It does not unify all forces. Instead, it isolates a single puzzle and proposes a single answer:

\textit{What if mass eats the vacuum?}

That is: what if mass absorbs momentum from the zero-point field, drawing in radiation in a steady inward flow? That inflow could explain why light bends near massive objects, why light shifts frequency in a gravitational well, and why these effects match those predicted by curved spacetime.

If this picture holds, then gravity isn’t geometry. It’s the vacuum responding to the hunger of mass.

...
The implications of this hypothesis will be tested in \textbf{Section 7}.
...
In this section we introduce the second of our two assumptions. The idea that gravity obeys equations analogous to those of electromagnetism was first proposed by Oliver Heaviside in 1893 \comment{citation needed — Heaviside's original paper}, predating general relativity. Following Heaviside, we assume that gravity obeys the same set of equations as those that govern electromagnetism, up to a numeric factor in the coupling constant. While the Maxwell equations were originally written for charge and charge currents, we replace these with mass and mass currents acting as sources. In natural units (\( c = 1 \)):
...
In the rest frame of the mass, these sectors decouple. One set of equations governs the gravitational field sourced by the mass, while another describes the radiation field. This separation enables a detailed analysis of the radiation field independently of the static component.
...
\noindent as expected. Note that while the point mass idealization suffices for recovering Newtonian gravity, in \textbf{Section 5} we refine this to a shell distribution (equation \ref{eq:delta_shell}) to address the gravitational self-energy problem.
...
Choosing the simplest mass distribution, a uniform spherical shell at radius \(R\)R, the self-energy integral yields:
...
Following Rohrlich (see Section~\ref{sec:toymodel}), if the energy absorbed from the ZPR can offset the divergence in the gravitational self-energy, then the particle is stable against collapse. We first analyze the energy density and total energy of the system, then examine whether this energy balance leads to an equilibrium. Finally, we turn to momentum conservation, where we derive an effective ``Ohm's law'' relating solenoidal fields and currents.
...
\begin{equation}
U_I = \frac{1}{16\pi} \int |\vec{E}_I|^2 \, d^3x \sim \int \frac{Gm^2}{r^4} \, d^3x
\end{equation}
...
- \textbf{Less Idealized Models}  Currently our model of a massive particle is as simple as possible. It would be interesting to study less idealized models.

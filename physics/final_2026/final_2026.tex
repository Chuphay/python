...
In the last subsection, we derived Newtonian gravity from the irrotational equations, in this subsection we will derive the equations of radiation from the solenoidal solutions.
...
\noindent so that $\langle \vec{J}_S \times \vec{B}_S \rangle = \sigma \langle \vec{E}_S \times \vec{B}_S \rangle$. Using the result from equation~\eqref{eq:momentum}, the radial component of the stochastic average of the Poynting-type integrand is:

\begin{equation}
\hat{r} \cdot \langle \vec{E}_S \times \vec{B}_S \rangle = -\sum_{l,m} \int d\omega\, \frac{4|a|^2}{\omega^2\, r^2}\, |\vec{X}_{lm}|^2
\end{equation}

\noindent The gravitational force density is also directed inward:

\begin{equation}
\rho \vec{E}_I = -m\delta^3(\vec{x})\, \frac{Gm}{r^2}\, \hat{r}
\end{equation}

\noindent For these to balance, $\sigma$ must be negative: the current opposes the field, representing absorption. The factor of $1/r^2$ appears on both sides and cancels, leaving:

\begin{equation}
\sigma = -\frac{m}{\hbar}\, \delta^3(\vec{x})
\end{equation}

\noindent This is a delta-function conductivity concentrated at the origin, or more precisely, at the shell $r = R$. It defines an effective Ohm's law in which the mass acts as a point-like absorber of incoming radiation. The solenoidal Maxwell equation (using the normalization adopted in Section~\ref{subsec:decomp}) becomes:

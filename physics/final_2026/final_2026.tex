...
In \textbf{Section 2}, we introduce the hypothesis: that mass acts as a sink for radiation and in particular from a pervasive stochastic zero-point radiation field. We first present the idea intuitively, anchoring the theory in a physical picture, and then spell it out mathematically.

In \textbf{Sections 3 and 4}, we lay out the two assumptions underlying the hypothesis. By “assumptions,” we mean elements already present in the scientific literature, perhaps unconventional but not novel to this work. \textbf{Section 3} introduces the stochastic zero-point radiation (Z.P.R.) field as the physical manifestation of the vacuum. \textbf{Section 4} adopts the Maxwell equations as the correct mathematical framework for describing gravity. 
...
\begin{itemize}
\item \textbf{Assumptions} are drawn from existing theoretical frameworks (even if unconventional)
\item \textbf{Hypothesis} refers to the novel claim proposed in this work
\end{itemize}
...
Taken together, these results support the view, held by Boyer and others, that the vacuum is in fact an active medium, filled with fluctuating modes that persist even at \( T = 0 \).  Importantly, the above results are achieved \emph{without quantization}. All results are derived using classical analysis.


We therefore, assume that the vacuum is filled with a classical, Lorentz-invariant random electromagnetic radiation field even at absolute zero.  As pointed out earlier, here when we say "Assumption" we mean we posit that a seperate theory is useful to this theory, we do not try and prove its claims here, we only assume that it is correct.



\subsection{The Stochastic Zero-Point Radiation Field}
...
where $\hat{\epsilon}(\vec{k}, \lambda)$ are polarization vectors satisfying: 
\begin{equation}
...
and $W_\lambda(\vec{k})$ are complex Gaussian random variables with correlation properties:
...
\subsubsection{Note on Averaging Conventions}

Before proceeding, we clarify an important distinction between classical Jackson electrodynamics and SED:

\begin{itemize}
	\item \textbf{Jackson (time-averaging):} Fields are assumed to have time dependence $e^{-i\omega t}$, and physical quantities like the Poynting vector are obtained by time-averaging. For fields $\vec{E} = \vec{E}_0 e^{-i\omega t}$ and $\vec{B} = \vec{B}_0 e^{-i\omega t}$, the time-averaged Poynting vector is $\langle \vec{S} \rangle_{\text{time}} = \frac{c}{8\pi} \text{Re}(\vec{E} \times \vec{B}^*)$, where the complex conjugate arises from the time-averaging procedure.
	
	\item \textbf{SED (stochastic averaging):} Fields contain stochastic variables $W_{l,m}(\omega)$ and we average over the ensemble of random field realizations. We write $\vec{E} = \vec{E}_+ + \vec{E}_+^*$ where $\vec{E}_+$ contains $W$ and $\vec{E}_+^*$ contains $W^*$. The stochastic average $\langle \vec{E} \times \vec{B} \rangle$ involves terms like $\langle \vec{E}_+ \times \vec{B}_+^* \rangle$, where the correlation function $\langle W_{l,m}(\omega) W_{l',m'}^*(\omega') \rangle = \delta_{ll'}\delta_{mm'}\delta(\omega-\omega')$ determines which terms survive.
\end{itemize}

Also, Boyer talks about this a lot and points to a paper by Einstein and Hopf.


\subsubsection{Derivation of Energy per Mode}
...
Using the correlation function $\langle W_\lambda(\vec{k}) W_{\lambda'}^*(\vec{k}') \rangle = \delta_{\lambda\lambda'} \delta^3(\vec{k} - \vec{k}')$:
...
This decomposition reflects a meaningful physical distinction between the \textbf{static gravitational attraction} and the \textbf{propagating radiation} field. Though derivable from the Maxwell equations, we list the continuity equation separately as it plays a direct role in what follows.
...
\subsection{Field Momentum in SED}
...
As mentioned in subsection earlier,  Both approaches lead to $\vec{E} \times \vec{B}^*$ appearing in the final expressions, but for different physical reasons.

\comment{obviously, find the subsection earlier (when we introduce ZPR)}

\subsection{hopefully the editing flowed}

\subsubsection{Full Field Expansion with Stochastic Coefficients}
...
\subsection{Confused about where we are}
...
\comment{This whole section needs to be re-written with the assumption of  the new stuff. We need to talk about including a model of a massive object here as well ... so there's a lot going on}
...
\subsection{Lower Bound 2: Minimum Radius via Gravitational Self-Energy}

We seek to establish a natural length scale for a massive particle by considering its gravitational self-energy. Consider a spherically symmetric mass distribution with total mass $m$ and density $\rho(\vec{x})$. The gravitational self-energy is:

\begin{equation}
U = -\frac{1}{2} \int \frac{G m \rho(\vec{x})}{r} \, d^3x
\end{equation}

If we assume the particle is purely gravitational in origin—that its rest mass arises entirely from gravitational binding energy—then this self-energy must equal the rest mass energy:

\begin{equation}
U = m
\end{equation}

We note that the more fundamental relativistic expression is $U^2 = m^2$ (or equivalently, $p_\mu p^\mu = m^2$ in four-vector notation), which we may utilize in subsequent analysis.

For a uniform spherical shell of radius $R$, direct integration yields:

\begin{equation}
R = \frac{G m}{2}
\end{equation}

Other mass distributions (uniform sphere, Gaussian profile, etc.) yield different numerical prefactors, but all give the same order of magnitude. In general, we require:
...
We interpret this as a \textbf{minimum radius} for a stable gravitational mass distribution. This scale is analogous to the classical electron radius, where electromagnetic self-energy becomes comparable to rest mass.

\comment{The numerical prefactor depends on the specific geometry assumed. Since we are constructing a model rather than deriving from first principles, different reasonable choices yield factors differing by $\mathcal{O}(1)$. We work with the general magnitude $R \sim Gm$ throughout this paper.}
...
We also note that $U^2 = m^2$ is a more natural expression, and we will be using that in the future (maybe).
...
\comment{again, this whole section needs to be re-written on the next pass. With attention paid to setting up a toy model of a massive object}
...
As noted in the previous section \textbf{Section 5} we need to use a toy model. And we will choose the spherical shell. So that, if we ever say "on shell" it will be clear.

\comment{We should mention that we will be using the "toy" model from the previous section and threfore restricting our attention to eht surface of the particle or being "on the shell"}
...
\comment{Using the heuristics we developed in subsection ~\ref{subsec:heuristic}, that were admittedly slightly fishy but we had decided were not fatal to the theory, namely that}
...
\noindent And of course just as we had "numerical prefactors" in section about mass r = R, here we do suppress numerical prefactors and absorb them into the definition of the model.
...
is actually a call to use our model and be "on shell", such that we do not actually try to integrate over spheres smaller than the smallest radius that was outlind ine \textbf{Section ?}
...
This expression defines a delta-function conductivity concentrated at the origin, but knowing we actually mean the shell. Thus this is an effective Ohm's law in which the mass acts as a point-like, perfect absorber for incoming radiation as long as we are outside of the radius  in \textbf{Section ?}.  We can re-write the relevant Maxwell equation in light of this:

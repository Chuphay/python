...
We do not specify the microscopic mechanism by which mass couples to the ZPR field. The implications of this hypothesis are tested in \textbf{Section 7}.
...
We now evaluate whether the inward radiation flow proposed in \textbf{Section 2} can stabilize a massive particle against gravitational collapse. Specifically, we ask whether the energy absorbed from the ZPR can counterbalance the gravitational self-energy that would otherwise drive collapse.
...
Considering only the irrotational contribution:
\begin{equation}
U_I = \frac{1}{4 \pi G} \int |\vec{E}_I|^2 \, d^3x \sim \int \frac{Gm^2}{r^4} \, d^3x
\end{equation}

This integral diverges, confirming that a massive particle would collapse under its own gravitational self-energy without a stabilizing mechanism. \comment{verify the $E_I$ integral in next pass}

The hypothesis of this paper is that the radiation field provides precisely this stabilization. Including both irrotational and solenoidal contributions, the total energy density is:
...
Integrating over $k$ and suppressing numerical prefactors, which are absorbed into the definition of the model (as in Section~\ref{sec:toymodel}), the total energy becomes:

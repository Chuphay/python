...
\noindent and identically for $W_{lm}^M$, with $\langle W_{lm}^E\, W_{l'm'}^{M*} \rangle = 0$. For a massive particle absorbing incoming radiation, we set $f_l(kr) = g_l(kr) = h_l^{(2)}(kr)$, since $h_l^{(2)}(kr)\, e^{-i\omega t} \sim e^{-i(kr + \omega t)}/(kr)$ at large $r$, representing ingoing waves.

Since there is no physical reason to prefer electric-type over magnetic-type modes in the vacuum, we impose the symmetry condition $|a_E|^2 = |a_M|^2 \equiv |a|^2$.
...
\begin{equation}
\boxed{
	\hat{r} \cdot \langle \vec{P}_{\text{field}} \rangle = -\frac{1}{\pi} \int d^3x \int d\omega \sum_{l,m} \frac{|a|^2}{\omega^2\, r^2}\, |\vec{X}_{lm}|^2
}
\label{eq:momentum}
\end{equation}
...
\begin{equation}
\boxed{
	\langle U \rangle = \frac{1}{\pi} \int d^3x \int d\omega \sum_{l,m} \frac{|a|^2}{\omega^2\, r^2}\, |\vec{X}_{lm}|^2
}
\label{eq:energy}
\end{equation}

\noindent Comparing equations \eqref{eq:momentum} and \eqref{eq:energy}, we see that $\hat{r} \cdot \langle \vec{P}_{\text{field}} \rangle = -\langle U \rangle$, consistent with the momentum--energy relation for incoming radiation (with $c = 1$).

The extension of the energy calculation beyond the radiation zone involves more intricate mathematics; we defer this to future work.


\newpage

\section{rederive newton}
\subsection{Newton's Gravity (Irrotational Part of the Maxwell Equations)}

For completeness, and to introduce notation used later, we derive the gravitational field of a static mass distribution. Consider a source

$$\rho = m\,\delta^3_x$$

where we are deliberately non-committal about the precise form of $\delta^3_x$; for the present purposes it suffices that outside some radius $R$ it behaves as $\delta^3_x = \delta(\vec x)$.

The irrotational part of the Maxwell equations reads

$$\nabla \cdot \vec{E}_I = -4\pi G\rho, \qquad \nabla \times \vec{E}_I = 0.$$

Expanding $\vec{E}_I$ in vector spherical harmonics,

$$\vec{E}_I = \sum_{l,m}\left[a_{lm}\,\vec{Y}_{lm} + b_{lm}\,(\hat r \times \vec{X}_{lm}) + c_{lm}\,\vec{X}_{lm}\right],$$

and applying the curl identities from the previous section, the condition $\nabla \times \vec{E}_I = 0$ yields, upon equating the $\vec{Y}_{lm}$, $\vec{X}_{lm}$, and $(\hat r \times \vec{X}_{lm})$ components independently:

$$c_{lm} = 0, \qquad a_{lm} = -\frac{i}{\sqrt{l(l+1)}}\frac{d}{dr}(r\,b_{lm}).$$

The divergence equation then gives

$$-4\pi G\,\rho_{lm} = \frac{1}{r^2}\frac{d}{dr}(r^2 a_{lm}) - i\sqrt{l(l+1)}\,\frac{b_{lm}}{r}\,.$$

Substituting the expression for $a_{lm}$ and defining

$$B_{lm} \equiv \frac{i\,r\,b_{lm}}{\sqrt{l(l+1)}}\,,$$
...
Outside the source ($r > R$, where $\rho_{lm} = 0$), the homogeneous equation admits power-law solutions $B \sim r^n$ with $n(n+1) = l(l+1)$, giving $n = l$ or $n = -(l+1)$. Requiring regularity at infinity, the exterior solution takes the form

$$B_{lm}(r) = \frac{\alpha_{lm}}{r^{l+1}}\,.$$

To fix $\alpha_{lm}$, we multiply (\ref{eq:radial}) by $r^{l+2}$ and observe the identity

$$r^l \frac{d}{dr}\!\left(r^2 \frac{dB}{dr}\right) - l(l+1)\,r^l\,B = \frac{d}{dr}\!\left[r^{l+2}\frac{dB}{dr} - l\,r^{l+1}B\right],$$

which recasts the equation as a total derivative:

$$\frac{d}{dr}\!\left[r^{l+2}\frac{dB_{lm}}{dr} - l\,r^{l+1}B_{lm}\right] = 4\pi G\,\rho_{lm}\,r^{l+2}\,.$$

Integrating from $r = 0$ (where regularity kills the boundary term) to any $r = R' > R$ (where we can use the exterior solution), the left side evaluates to $-(2l+1)\,\alpha_{lm}$ and the right side gives $4\pi G\,q_{lm}$, where the multipole moments are defined as

$$q_{lm} \equiv \int Y_{lm}^*(\theta,\phi)\,r^l\,\rho(\vec x)\,d^3x = \int \rho_{lm}(r)\,r^{l+2}\,dr\,.$$

Thus

$$B_{lm} = -\frac{4\pi G\,q_{lm}}{(2l+1)\,r^{l+1}}\,.$$

Inverting the definition of $B_{lm}$ to recover $b_{lm}$, computing $a_{lm}$ from the curl-free condition, and assembling the expansion, the gravitational field outside the source is

$$\vec{E}_I(\vec x) = \sum_{l,m}\frac{-4\pi G\,q_{lm}}{(2l+1)\,r^{l+2}}\left[(l+1)\,\vec{Y}_{lm} + i\sqrt{l(l+1)}\,(\hat r \times \vec{X}_{lm})\right].$$

For a point source $\rho = m\,\delta(\vec x)$, only $q_{00} = m/\sqrt{4\pi}$ survives, and this reduces to

$$\vec{E}_I(\vec x) = -\frac{mG}{r^2}\,\hat r, \qquad r > R,$$
...
This is the standard radial Poisson equation in a spherical harmonic basis (see Jackson, \S 3.9). The exterior solution ($r > R$) is


$$ B_{lm} = - \frac {4 \pi G }{2l + 1} \frac {q_{lm}}{r^{l+1}} $$ 
...
Now we can write out the final solution:
